...
MVP 포함 기능에 한정한다. MVP 제외 기능(상점, 포인트 등)은 이 문서에서 제거하거나 v2.0 예정으로 분리한다.
...
| 메서드 | 경로 | 설명 | 인증 |
|--------|------|------|------|
| `POST` | `/api/v1/disputes` | 사건 생성 (방 생성 시 함께 생성) | 🔒 |
...
- **처리 정책:**
  ...
  - 카카오 로그인 완료 시 약관 동의로 간주한다 (별도 약관 동의 페이지 이동 없음)
- **확정 필요:**
  - 닉네임 자동 생성 패턴 및 길이
...
### Personal Analysis (단독 판결)
- [ ] 단독 판결 결과 응답 필드 범위 확정 (PERSONAL_ANALYSIS.md §8 — TODO)
- [ ] 1:1 판결 결과 구조와 공유 여부 (verdictScore 포함 여부 등)
- [ ] 단독 판결 후 상대방 초대로 전환 시 흐름 처리 방식
